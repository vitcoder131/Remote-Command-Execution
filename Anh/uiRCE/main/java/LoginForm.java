...
public class LoginForm extends JFrame {
    ...
    public LoginForm() {
        ...
        // Đường phân cách "OR" 
        JPanel orPanel = createOrSeparator(getAppFont(15, Font.PLAIN));
        orPanel.setAlignmentX(Component.CENTER_ALIGNMENT);

        // Nút SIGN IN (Bo góc, căn giữa và có Hover)
        Color signInBaseColor = new Color(220,220,220);
        RoundedButton signInButton = new RoundedButton("HAVE AN ACCOUNT? SIGN IN", signInBaseColor, Color.BLACK);
        signInButton.setFont(getAppFont(16, Font.BOLD));
        signInButton.setMaximumSize(new Dimension(350, 48)); 
    ...
    // ======= HÀM TẠO ĐƯỜNG PHÂN CÁCH "OR" CHUYÊN NGHIỆP =======
    private JPanel createOrSeparator(Font font) {
        JPanel orPanel = new JPanel(new GridBagLayout());
        orPanel.setOpaque(false);
        orPanel.setMaximumSize(new Dimension(400, 20));
        ...
        // 1. Đường kẻ trái
        JSeparator sepLeft = new JSeparator(SwingConstants.HORIZONTAL);
        sepLeft.setForeground(Color.LIGHT_GRAY);
        gbc.weightx = 1.0; // Chiếm hết không gian ngang
        gbc.gridx = 0;
        orPanel.add(sepLeft, gbc);

        // 2. Chữ "OR"
        JLabel orLabel = new JLabel("OR");
        orLabel.setFont(font);
        orLabel.setForeground(Color.GRAY);
        gbc.weightx = 0; // Giữ kích thước ban đầu
        gbc.gridx = 1;
        orPanel.add(orLabel, gbc);

        // 3. Đường kẻ phải
        JSeparator sepRight = new JSeparator(SwingConstants.HORIZONTAL);
        sepRight.setForeground(Color.LIGHT_GRAY);
        gbc.weightx = 1.0; // Chiếm hết không gian ngang
        gbc.gridx = 2;
        orPanel.add(sepRight, gbc);
